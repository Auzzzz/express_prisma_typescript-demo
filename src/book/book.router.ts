...
bookRouter.get("/:id", async (request: Request, response: Response) => {
  ...
  try {
    ...
    return response.status(404).json("No book found by id");
  } catch (error: any) {
    return response.status(500).json(error.message);
  }
});
...
bookRouter.put(
  ...
  async (request: Request, response: Response) => {
    ...
    try {
      const book = request.body;
      const updatedBoook = await BookService.updateBook(book, id);
      return response.status(201).json(updatedBoook);
    } catch (error: any) {
      return response.status(500).json(error.message);
    }
  }
...
bookRouter.delete("/:id", async (request: Request, response: Response) => {
    ...
    try{
        ...
        return response.status(204).json("Book was deleted")
    } catch (error: any){
